...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('exportTruthTableCsv')?.addEventListener('click', () => {
    if (client.isBackendAvailable) {
      ...
      const table = PLACore.generateTruthTable(currentCompiled);
      let csv = 'Minterm,A,B,C,D,Active_Product_Terms,' + currentCompiled.outputIds.map(o => `Output_${o}`).join(',') + '\n';
      table.forEach(r => {
        csv += `${r.minterm},${r.A},${r.B},${r.C},${r.D},"${r.active_terms.join(';')}",` + currentCompiled.outputIds.map(o => r.outputs[o] || 0).join(',') + '\n';
      });
      downloadBlob(csv, 'pla_truth_table.csv', 'text/csv');
    }
  });

  document.getElementById('exportTruthTableJson')?.addEventListener('click', () => {
    const table = PLACore.generateTruthTable(currentCompiled);
    downloadBlob(JSON.stringify(table, null, 2), 'pla_truth_table.json', 'application/json');
  });
  ...
  document.getElementById('exportTestCasesCsv')?.addEventListener('click', () => {
    if (client.isBackendAvailable) {
      ...
      let csv = 'ID,Title,Category,A,B,C,D,Expected_Outputs,Description\n';
      const tests = [
        { id: 1, title: 'Normal 1: Quiescent (0000)', category: 'normal', a:0, b:0, c:0, d:0, expected: '{"Y1":0,"Y2":0,"Y3":1,"Y4":0}', desc: 'Ground state' },
        { id: 2, title: 'Normal 2: Single Bit C (0010)', category: 'normal', a:0, b:0, c:1, d:0, expected: '{"Y1":1,"Y2":0,"Y3":0,"Y4":0}', desc: 'C active' },
        { id: 3, title: 'Normal 3: Single Bit B (0100)', category: 'normal', a:0, b:1, c:0, d:0, expected: '{"Y1":1,"Y2":0,"Y3":0,"Y4":0}', desc: 'B active' },
        { id: 4, title: 'Normal 4: Dual Bit B,C (0110)', category: 'normal', a:0, b:1, c:1, d:0, expected: '{"Y1":0,"Y2":1,"Y3":0,"Y4":0}', desc: 'B,C active' },
        { id: 5, title: 'Normal 5: Single Bit A (1000)', category: 'normal', a:1, b:0, c:0, d:0, expected: '{"Y1":1,"Y2":0,"Y3":0,"Y4":0}', desc: 'A active' },
        { id: 6, title: 'Normal 6: Dual Bit A,C (1010)', category: 'normal', a:1, b:0, c:1, d:0, expected: '{"Y1":0,"Y2":1,"Y3":0,"Y4":0}', desc: 'A,C active' },
        { id: 7, title: 'Normal 7: Dual Bit A,B (1100)', category: 'normal', a:1, b:1, c:0, d:0, expected: '{"Y1":0,"Y2":1,"Y3":0,"Y4":1}', desc: 'A,B active' },
        { id: 8, title: 'Normal 8: Dual Bit A,B with D (1101)', category: 'normal', a:1, b:1, c:0, d:1, expected: '{"Y1":0,"Y2":1,"Y3":0,"Y4":0}', desc: 'A,B,D active' },
        { id: 9, title: 'Normal 9: Tri-Bit A,B,C (1110)', category: 'normal', a:1, b:1, c:1, d:0, expected: '{"Y1":1,"Y2":1,"Y3":0,"Y4":1}', desc: 'A,B,C active' },
        { id: 10, title: 'Normal 10: Quad-Bit (1111)', category: 'normal', a:1, b:1, c:1, d:1, expected: '{"Y1":1,"Y2":1,"Y3":1,"Y4":0}', desc: 'All active' },
        { id: 11, title: 'Edge 1: Saturation State (All Ones)', category: 'edge', a:1, b:1, c:1, d:1, expected: '{"Y1":1,"Y2":1,"Y3":1,"Y4":0}', desc: 'Max fan-in' },
        { id: 12, title: 'Edge 2: Ground State (All Zeros)', category: 'edge', a:0, b:0, c:0, d:0, expected: '{"Y1":0,"Y2":0,"Y3":1,"Y4":0}', desc: 'Ground state' },
        { id: 13, title: 'Edge 3: Interlock Threshold (1100)', category: 'edge', a:1, b:1, c:0, d:0, expected: '{"Y1":0,"Y2":1,"Y3":0,"Y4":1}', desc: 'Threshold' },
        { id: 14, title: 'Edge 4: Parity Inversion Edge (0111)', category: 'edge', a:0, b:1, c:1, d:1, expected: '{"Y1":0,"Y2":1,"Y3":0,"Y4":0}', desc: 'Single 0' },
        { id: 15, title: 'Edge 5: Dynamic Hazard Boundary (1001)', category: 'edge', a:1, b:0, c:0, d:1, expected: '{"Y1":1,"Y2":0,"Y3":0,"Y4":0}', desc: 'Hazard edge' }
      ];
      tests.forEach(t => {
        csv += `${t.id},"${t.title}",${t.category},${t.a},${t.b},${t.c},${t.d},"${t.expected}","${t.desc}"\n`;
      });
      downloadBlob(csv, 'pla_test_cases.csv', 'text/csv');
    }
  });
  ...
  document.getElementById('exportDatasetCsv')?.addEventListener('click', () => {
    if (client.isBackendAvailable) {
      ...
      let csv = 'Sample_ID,A,B,C,D,Expected_Y1,Expected_Y2,Expected_Y3,Expected_Y4,Has_Fault,Fault_Type\n';
      for (let i = 0; i < 32; i++) {
        const a = Math.round(Math.random()), b = Math.round(Math.random()), c = Math.round(Math.random()), d = Math.round(Math.random());
        const y1 = a ^ b ^ c;
        const y2 = ((a & b) | (b & c) | (a & c)) ? 1 : 0;
        const y3 = ((a && b && c && d) || (!a && !b && !c && !d)) ? 1 : 0;
        const y4 = (a && b && !d) ? 1 : 0;
        csv += `S${String(i+1).padStart(3,'0')},${a},${b},${c},${d},${y1},${y2},${y3},${y4},false,None\n`;
      }
      downloadBlob(csv, 'pla_synthetic_dataset.csv', 'text/csv');
    }
  });
